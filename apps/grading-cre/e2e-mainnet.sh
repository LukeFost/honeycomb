...
#  4. x402-gated spawn: each grader VM is launched ONLY after a real x402 USDC settle.
...
cre_relay(){ local f=$1 label=$2 tx
  tx=$(cd $GR && cre workflow simulate grading-workflow --non-interactive --target mainnet-settings --trigger-index 0 --http-payload "$f" --broadcast 2>/tmp/e2e_err | grep -oE '"txHash":"0x[0-9a-f]+"' | grep -oE '0x[0-9a-f]+' | head -1)
...
grade_record(){
  ...
  echo "  ── $label (agent $aid · $sname) — x402-summon the VM, grade in-enclave ──"
  grade=$(bash $GR/grader/summon_enclave.sh "$sname" $JOB $aid $VM_DIGEST) || die "$label: x402-summon + in-VM grading failed"
...
RTX=$(cd $GR && cre workflow simulate grading-workflow --non-interactive --target mainnet-settings --trigger-index 1 --broadcast 2>/tmp/e2e_err | grep -oE '"txHash":"0x[0-9a-f]+"' | grep -oE '0x[0-9a-f]+' | head -1)
...
if [ "$DEC" = "buy" ]; then
  ...
  SWAP_OUT=$(cd $SV && CRE_TARGET=staging-settings CRE_ETH_PRIVATE_KEY=${MAKER_PK#0x} cre workflow simulate strategy-workflow --non-interactive --trigger-index 0 --broadcast -e $ENVF 2>/tmp/e2e_err)
  SWAPTX=$(echo "$SWAP_OUT" | python3 -c "
import sys,json,re
m=re.search(r'\{.*\}', sys.stdin.read(), re.S); d=json.loads(m.group(0)) if m else {}
r=[x for x in d.get('results',[]) if x.get('vault','').lower()=='${VAULT,,}']
print(r[0]['txHash'] if r and r[0].get('txHash') else '')" 2>/dev/null)
